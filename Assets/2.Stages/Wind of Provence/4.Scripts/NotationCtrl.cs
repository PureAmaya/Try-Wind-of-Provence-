...
//提前显示下一个（如果在时间区间较短）
    ...
    private void ShowAheadForJunna()
    {
        ...
        if (whichIntervalToUseForJunna == 0) 
        {
            ...
            {
                //开头的四分音符（有线）
                notesPoolForJunna[0].GetTransform().position = GetUIToWordPos(JunnaNoteLocation[0]);
                //中间的正常八分音符（有线） 
                notesPoolForJunna[2].GetTransform().position = GetUIToWordPos(JunnaNoteLocation[1]);
                notesPoolForJunna[3].GetTransform().position = GetUIToWordPos(JunnaNoteLocation[2]);
                //第四小节的八分音符（有点）
                notesPoolForJunna[1].GetTransform().position = GetUIToWordPos(JunnaNoteLocation[3]);
        ...
        if (videoPlayer.frame >= JunnaReadable[JunnaInterval[^2] + 1] - 60)
        {
            if (notesPoolForJunna[2].GetTransform().position.y < -50f)
            {
                ...
                {
                    notesPoolForJunna[2 + i].GetTransform().position = GetUIToWordPos(JunnaNoteLocation[i]);
                }

                //第四小节的八分音符（有点）
                notesPoolForJunna[1].GetTransform().position = GetUIToWordPos(JunnaNoteLocation[number]);
...
//按照数目要求，把八分音符放上去
  ...
  /// <param name="leftNotes">所有的完整亚组弄完之后，剩下的不足八个的，Junna = 0，仍然是完整的亚组，也为0</param>
    ...
    { 
        ...
        if (whichIntervalToUseForCharacter == 0)
        {
            ...
            for (int i = 1; i < noteCount; i++)
            {
                ...
                myLocation[i] = GetUIToWordPos(characterNoteLocation[i - 1]);
                targetLocation[i] = GetUIToWordPos(characterNoteLocation[i]);
            }

            //补上第一个
            targetFrame[0] = characterReadable[0];
            targetLocation[0] = GetUIToWordPos(characterNoteLocation[0]);
        }
        else
        { 
          //正常处理计算速度
            for (int i = 0; i < noteCount; i++)
            {
                ...
                targetLocation[i] = GetUIToWordPos(characterNoteLocation[i]);
              if(i >= 1)  myLocation[i] = GetUIToWordPos(characterNoteLocation[i - 1]);
            }
        }
     
        //第一个修正
        myFrame[0] = initialFrame;
        myLocation[0] =GetUIToWordPos(panDingsInitialLocation[character]);
    ...
    private void Initialization()
    {
        ...
        //判定方块回车设定
        panDingSquares[0].SetEnterAndEnd(GetUIToWordPos(panDingsInitialLocation[0]),
            GetUIToWordPos(JunnaNoteLocation[^1]) + Vector2.right * 0.4f);
        panDingSquares[1].SetEnterAndEnd(GetUIToWordPos(panDingsInitialLocation[1]),
            GetUIToWordPos(MasakoNoteLocation[^1]) + Vector2.right * 0.4f);
    ...
    private void HowMasakoNoteShow(int number,bool isFirst = false)
    {
        ...
        switch (number)
        {
            ...
            case <= 8:
                ...
                for (int i = 0; i < number; i++)
                {
                    notesPoolForMasako[i].GetTransform().position = GetUIToWordPos(MasakoNoteLocation[i]);
                }
